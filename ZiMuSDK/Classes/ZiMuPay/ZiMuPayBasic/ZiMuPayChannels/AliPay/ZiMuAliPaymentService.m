...
@implementation ZiMuAliPaymentService

+ (instancetype)shareInstance {
    static ZiMuAliPaymentService * instance = nil;
    static dispatch_once_t  onceToken ;
    dispatch_once(&onceToken, ^{
        instance = [[ZiMuAliPaymentService alloc] init];
    });
    return instance;
    
}

- (void)payWithOrder:(NSObject *)order secheme:(NSString *)secheme result:(ZiMuPayResultHandle)resultHandle {
...
- (void)analysisResultDic:(NSDictionary *)resultDic {
...
#endif
    ZiMuPayResultStatus status;
    NSInteger resultStatus = [resultDic[@"resultStatus"] integerValue];
    if (resultStatus == 6001) {
        status = ZiMuPayResultStatusCancel;
    } else if (resultStatus == 9000) {
        status = ZiMuPayResultStatusSuccess;
    } else if (resultStatus == 8000) {
        status = ZiMuPayResultStatusProcessing;
    } else {
        status = ZiMuPayResultStatusFailure;
    }
    ...
    NSError *error = [NSError errorWithDomain:NSStringFromClass(self.class) code:kZiMuPayErrorCode userInfo:@{NSLocalizedDescriptionKey:message}];
